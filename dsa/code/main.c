    ...
    while(1){
        ...
        if(plat==1){
            if(strcmp(fn,"add_post")==0){
                ...
                scanf("\n%[^\n]s",user);
                scanf("\n%[^\n]s",content);
                check = addComment(user,content);
                if(check==false) printf("Comment not added,No last viewed post!!");
            }
                ...
                scanf("\n%[^\n]s\n",reply);
                scanf("%d",&no);
                check = addReply(user,reply,no);
                if(check==false) printf("Reply not added!!");
            }
